...
- Remove folder tags and reject attempts from search, contacts, deep links, or stale notifications to open chats outside `main`.
...
Add one shared selector for locating the `main` folder. Feed only its ID into the existing folder-list components so Telegram's current folder manager remains the source of membership and ordering. Reuse the same membership check in the shared chat-opening action and close any cached non-`main` chat after folder membership loads.
